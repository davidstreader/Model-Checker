...
public class JSONToASTConverter {
    ...
    public AbstractSyntaxTree convert(JSONObject ast){
        ...
        return new AbstractSyntaxTree(processes, operations, null, variableMap);
    }
    ...
    public ProcessNode convertProcessNode(JSONObject json){
        ...
        JSONObject jsonVarSet = json.optJSONObject("variables");
        ...
        JSONObject jsonLocation = json.getJSONObject("location");
        Location location = convertLocation(jsonLocation);
        VariableSetNode variables = null;
        if (jsonVarSet != null) {
            variables = convertVariableSetNode(jsonVarSet, location);
        }
        ProcessNode node =  new ProcessNode(type, identifier, process, localProcesses, hiding, location);
        node.setVariables(variables);
        return node;
    }

    private VariableSetNode convertVariableSetNode(JSONObject json, Location location) {
        JSONArray jsonSet = json.getJSONArray("set");
        Set<String> stringSet = new HashSet<>();
        for (int i = 0; i < jsonSet.length(); i++) {
            stringSet.add(jsonSet.getString(i));
        }
        return new VariableSetNode(stringSet,location);
    }
