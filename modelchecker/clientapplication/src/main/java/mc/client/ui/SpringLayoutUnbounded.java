 ...
 * @author Jordan Smith
 */
public class SpringLayoutUnbounded<V, E> extends SpringlayoutBase<V,E>
{
    protected int currentIteration;
    protected int averageCounter;
    protected int loopCountMax = 4;
    protected Map<String, Set<GraphNode>> automata;
     ...
     */
    public SpringLayoutUnbounded(Graph<V,E> g,Map<String, Set<GraphNode>> automata_) {
        super(g);
        automata = automata_;
    }
     ...
     */
    public SpringLayoutUnbounded(Graph<V,E> g, Function<E, Integer> length_function)
    {
    ...
    private void testAverageDeltas() {
        ...
        if(Math.abs(dx) < .001 && Math.abs(dy) < .001) {
            //Shunt the different process models away from eachother
            if(automata.size() > 0) {

                double previousLength = 0;

                for (String currentAutomata : automata.keySet()) {
                    for (GraphNode vertex : automata.get(currentAutomata)) {
                        Point2D location = this.apply((V) vertex);
                        location.setLocation(location.getX()+ previousLength+20, location.getY());
                    }
