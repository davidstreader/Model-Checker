...
// Worry might this loop forever ?
            ...
            for (AutomatonEdge nh : newHidden) {
                hiddenEdges.add(nh);
            }
...
//.println("Edge Guard "+ edge.getGuard());
     ...
     *   This works in as much as it produces the correct Aut but not consistent
     *   ownership hence the Owners Rule will not give the correct Petri Net
     *
     *   edge ownership is the union of the "tracks" it is on!
     *   the owners Rule projects to these tracks
     *       disjoint owners => parSquare
     *   abstraction breaks this!
     *
     */
    ...
    private Automaton pruneHiddenNodes(Context context, Automaton autoIN, boolean cong)
        ...
        for (AutomatonNode n : nodes) {
            ...
            /* Only prune nodes connected to NO observabe event*/
            boolean del = true;
