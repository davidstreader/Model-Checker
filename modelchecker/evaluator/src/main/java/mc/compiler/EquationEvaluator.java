...
public class EquationEvaluator {
  ...
  // domains only used in Instantiate but built from processMap
   ...
   /* each time compile is pushed a new EquationEvaluator object is built and
     ...
     an interupt stops the pools Executor
    */
 ...
 */
  ...
  //REF    Instantiate inst = new Instantiate(processMap, globlFreeVariables, globlFreeVariables);
    ...
    try {status = startTest(  // recursive call down equation term
...
// must pass
        ...
        localStatus.addFailure("Forall fail");
            localStatus.addFailure(" free "+asString(outerFreeVariabelMap));
        ...
        if (((ImpliesNode) operation).getFirstOperation() instanceof ForAllNode) {
          ...
          if (status1.failCount != 0) {  // false ==> X  ~~> true
            ...
            if (status2.failCount > 0) {  //true==> false ~~> false
              ...
              failures.add("true ==> false");
              failures.addAll(status2.getFailures());
              System.out.println("  @====@ Failing 2 Implies 1->2" + operation.myString() + " " + asString(outerFreeVariabelMap) + " fail " + status2.getFailures());
                status.setFailures(failures);
      ...
      if (freeVariables.size() == 0) {
          status.setFailures(new ArrayList<String>());
          return status;
      }// called with a ground term so no looping
      if (updateVariables) { // only true for first call and forAll
        if (inst.end()) {
            status.setFailures(new ArrayList<String>());
            return status;
        }
      ...
      System.out.println(" Operation "+i+"  "+ operation.myString()+" passed "+ status.passCount + " next "+ asString(outerFreeVariabelMap));
