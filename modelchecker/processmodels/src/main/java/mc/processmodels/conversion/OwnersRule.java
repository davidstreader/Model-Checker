...
/*
 ...
 //System.out.println("stripStar root"+roots);
   ...
   String rs = roots.iterator().next();
   //System.out.println("rs = "+rs);
   List<Set<String>> newRoots = new ArrayList<>();
   Integer rindx = 1;
   for (PetriNetTransition tr: pout.getPlaces().get(rs).post()) {
   ...
   pout.removePlace(pout.getPlaces().get(rs));
