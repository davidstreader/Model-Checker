...
{\color{red}
Need to introduce the {\bf location of transitions} of a  automata. Parallel processes have disjoint locations. Sequential processes, choice and action prefixing have identical locations.  The {\sf hide}ing preserves the location of all observed transitions.
...
{\bf Suggestion 1 *Not So Good*}  Let {\sf Loc(A) = 1, Loc(B)=2} in above.   Add the root {\sf 0} to the TODO list. For each element in TODO list compute the surfaces if  we have  two transitions with disjoint location leaving  so compute $\sf \{0,1,2,4\}$. Remove $\sf 0$ from the TODO list and add nodes ${\sf \{1,2,4\}}$ to the TODO list.  When 
...
{\bf Suggestion 2 *Looks Good*} Step 1 approximation build separate Nets one for each location, duplicating synchronised events. Where {\bf twists} occur make cross product of places.
