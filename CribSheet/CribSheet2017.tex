...
\subsection{Bug 2  Broadcast communication}
...
\end{verbatim}
\subsection{Bug 3 restricted rendering }


\begin{verbatim}
const N = 3
automata {
             /*Bug in not rendering automata */
Sendb* = input-> sout! -> Sendb.
Receiveb* = sout? -> output->Receiveb.
Go = Sendb||Receiveb.

Buf1data = input[i:1..N] -> F[i],
   F[d:1..N] = output[d] -> Buf1data.
 
           /* not rendering Buf2a makes the compile fail 
 Performing layout for Buf2, remaining: 1
An exception was thrown that was not related to your script.
...
\subsection{Need for failure semantics}
\begin{verbatim}
automata {
