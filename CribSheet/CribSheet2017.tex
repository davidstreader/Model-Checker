...
{\bf Automata alone do not preserve concurrency information.}  By recording 


 Define a Location space where each location is a subset of primitive locations. Build a Place for each primitive location {\sf own} by building a projection of  the automata onto the location. In essence events at a location containing {\sf own} are preserved all other events are hidden. Finally the set of projected automata can be composed in parallel synchronising only the events that have been projected for the same underlying event. 
...
Whereas event refinement can be defined on Petri Nets.  Because of the addition of location the two models are equivalent but nonetheless trying to define event refinement on located automata or testing equivalence on Petri Nets is far from easy.  Several definitions of process equality have been defined on Petri Net semantics but they are difficult to understand and very hard to compare thurer ly. 
...
Petri Nets can be very much smaller than automata and also  support event refinement. Automata support a rich set of equivalences that can be justified by plausible testing semantics and easily represent reachability of events. 
...
Indistinguishable processes  can be formalised as \emph{testing equivalent}. Substitutive can be formalised are congruent w.r.t. process operators, from a practical perspective the most important being parallel composition.
 ...
 \subsection{Complete Trace equality   $=_{Tr^c}$ or in the tool {\sf \_\#\_} }
 ...
 \subsection{Failure equality  $=_{F}$ or in the tool {\sf \_*\_}  }
...
{\bf \large Quality Code}
The equation processor allows algebraic equational rules to be checked by building a set of test processes and then instantiating each variable to generate a ground equality that can be validated.  We currently use this to generate thousands of tests. This has improved the code quality significantly. 
...
\item Support for plug-in style development.
\end{enumerate}
