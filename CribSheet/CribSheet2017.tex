...
Although it is interesting to have a sound and complete inference system to reason about programs it should be borne in mind that some systems used  practice are not complete.   Both B and Event-B  define refineement as  forward simulation and do not use backward simulation  hence are inherently incomplete with respect to refinement and yet  have been shown to be  be of great use in practice.
What we believe to be of greater practical importance than completeness is that the inference system is
...
From our perspective we view the primary role of Formal Methods as the modelling of systems of interest. Having built the formal model we can then prove, or discover,  properties the model has. Such properties are only of practical interest in as much as the model is a accurate reflection of the actual systems of interest. To give us confidence that the models we use are an accurate reflection of the behaviour of the systems we are interest in we construct testing semantics. Consequently you should have no  confidence applying any results   if you have no confidence in that the system of interest actually interacts with the world around it in the manor prescribed by the testing semantics.

Any model, formal or otherwise, is an abstraction of the real system. Ideally models will be small and simple while still faithfully capturing the  aspects of the system we are interested in.  

Naturally if you change what you are interested in you may have to change the models you are using.  Here this can be achieved by changing the testing semantics. The moment you have two models of the same system the natural question is how are they related. The multiple models approach is quite standard in computer science. One, very abstract model, might be called a specification and a second, more concrete but still abstract, model an implementation. The relation between specifications and implementations is often formalised by refinement. Frequently both the specification and implementation use the same style of modelling, they differ only in their level of abstraction or  the detail they contain. Here we are interested in both refinement with in a single formalism and refinement between different, but related, formalisms.


\subsection{Initial event based modelling}
Our style of modelling can be described as based on \emph{hand shake events} and as is common we call such programs processes. We consider CSP, CCS and ACP as all using  hand shake events and like here make the following assumptions:
\begin{enumerate}
...
Here we are less concerned with representing systems exclusively as terms and constructing a sound and complete set of axioms  than in their operational semantics. Based on the operational semantics   we define sound algorithms for the rewriting of the semantics. This can be seen as what in the literature is called \emph{Visual verification}.


We wish to allow for the change of our models of a system by either changing the modelling language used or changing the operational semantics given to the terms.  Not only this but we wish to make such changes formally. This we can do because we have an abstract definition of testing and testing refinement hence using this as a common language to define different styles of modelling we are able to define a formal relation between the styles of modelling.

Interpreting specifications and implementations as predicate we have the common notion of refinement as logical implication.  From Logic we know that distinct logics can be related by Galois connections, commonly know as theory morphisms. Galois connections form a strong connection between theories that have well known and useful properties. Although Galois connections  ensuring a close relation between two theories we have found them sufficiently flexible to formalise interesting refinement steps.

\subsection{Secondary models}

Our primary formalism is of  \emph{symmetric hand shake events} and adopts the \emph{interleaving assumption}.

We will construct two models with out the interleaving assumption and show that they are equivalent. We do this by recording which sequential process owns and event and then building \emph{automata of owned event}  and \emph{Petri Nets of owned events}.


We also construct \emph{broadcast events}. In attempting to show the equivalence between hand shake processes \ie processes with hand shake events and broadcast processes we were forced to extend what hand shake processes modelled. The hand shake processes were extended to distinguish \emph{causality} between synchronising events.

First we extended hand shake event to being owned events and then extended them to being causal hand shake events. Quite naturally with could apply both extensions to have \emph{causal owned events}. As all four models can be related by Galois connections we are able to change what appears in our model as we move from specification to implementation. 
 


\subsection{Other Event based Approaches}
 Process calculi CSP, CCS, ACP and  Lotos  are  successful examples of event based formalisms. They have generated  a wide range of denotational semantics and process equivalences.  Each can be  given an axiomatic semantics to enable  reasoning about the processes. Although these approaches make central use of axioms some, notably CCS and ACP also construct an operational semantics on which both operations and equivalences can be defined.
...
We are interested in defining processes that have private state and only interact with other processes via {\bf events}.  Unlike the process calculi CSP, CCS, ACP and  Lotos  we do not   make use of the  axiomatic semantics to reason about the processes. Our non axiomatic  approach that loosely   follows the notion of \emph{Visual Verification}  from  Valmaries .. \cite{}, later we will make use of the axioms check (\emph{test}) the validity of our approach.  Testing is not verification and in an ideal setting once a theory has been thoroughly  tested it should be verified by, for example, encoding the theory a theorem prover. 

Most industrial software, even most life critical software, is tested not verified the idea of testing a theory might prove acceptable, if not to theorists, at least to industrialists.  Our approach  of model checking a theory could be viewed as taking an experimental approach to theory development. 
...
It will prove useful to define this operation on automata. As such the automata for the choice between two component processes can be built from the two  component automata simply by glueing together the start nodes of each process.  With out changing the behaviour of the process in any way we can also glue the two end nodes together.
...
Two processes can be composed in sequence using \verb|=>|. Just like with the choice operator we can define sequential composition by gluing nodes together. Consequently the automata \verb|A=>B| can be built by gluing the end node of the automata of process \verb|A| with the start node of automata \verb|B| 

Clearly action prefixing and sequential composition are closely related:

\begin{center}\verb$a->b->STOP = (a->STOP)=>(b->STOP) = (a->STOP)=>(b->STOP)=>STOP$\end{center}

in what follows we often make no distinction between such process specifications.
 Sequential composition should also satisfy the following axioms:
...
The  process \verb$(s->t->STOP)=>(a->STOP)+(b->STOP)$ is interesting in that the internal choice is made after the \verb$t$ event. Where as  process in the  \verb$(s->t->a->STOP)+(s->t->b->STOP)$ it  is made before the \verb$s$ event. Despite this  neither can be distinguished by any test. They are failure semantics equivalent. This is a direct consequence of the fact that you can not observe when a choice is made you can only \emph{infer} a choice must have been made some time in the past.  Because such process cannot be distinguished  how you choose to represent such a specification is a matter of taste.
...
\vspace{1ex}\emph{\bf A Petri Net} is a tuple  $\sf A \triangleq (P_A,S_A,T_A,Arcs_A,\alpha_A)$  where:
\begin{description}
\item[$\sf P_A$] is a set of places,
...
\item[$\sf T_A$] is a set of event labelled transitions
\item[$\sf Edges_A$] Edges join places to transitions and transitions to places. There can be multiple places joined to a single transition and multiple transition joined to a single place.
\newline
$\sf Edges_A\subseteq \{(p,t): p\in P_A, t\in T_A \}  \cup \{(t,p): p\in P_A, t\in T_A \}$
\item[$\sf \alpha_A$] is a set of event labels \hspace{\fill}$\Box$

\end{description}

As defined Petri Nets offer a richer semantic models that Automata. Specifically Petri Nets do not make use of the interleaving interpretation of parallel composition and hence can distinguish \verb$a->STOP||b->STOP$ from \verb$a->b->STOP[]b->a->STOP$ which have  identical interleaving semantics. Later we will show how to extend both semantics models so that they are equally expressive.

Let us note by introducing an internal choice operator we need to have a set of initial markings else we would need to alter the semantics of $\tau$ event from that used in CCS. (Although CSP dose exactly this we have decided not to.) The introduction of  sequential composition as well as action prefixing is facilitated by the explicit definition of successful termination.  
...
Parallel merge of two processes, parallel composition with no synchronisation is just the union of the component nets. Hence each automata node is corresponds to  a Petri Net marking. For the process considered a marking  is a pair of places, one element of the pair taken from each component sequential process. 
With the language considered so far all Petri Nets constructed will be one-safe. That is no more than one token at a time will be places on any of the Petri Net places. 
Thus each place can be thought to represent a predicate defining  the state of one of the component processes, the marking of a place is the assertion that the predicate is currently true. Each sequential process can be can be though to \emph{own}  a  set of places and transitions. This ownership information can be computed directly from the process term and hence could be added as an annotation to either the Petri Net or to the Automata.

In the diagram above the start node of the automata is coloured light blue and corresponds to the the pair of initially marked places that are also coloured light blue.


\subsection{The operational semantics of  Petri Nets}
We give an operational meaning to  Petri Nets  by defining their behaviour in terms of how  tokens can be moved around a Petri Nets. This is achieved by defining the {\sf Token Rule} that is a mapping from Petri Nets to automata.

 The \verb|TokenRule| maps Petri Nets to automata and the automata can be seen as the operational semantics of the Petri Net.  A transition can only be executed when all its pre places are marked and the result of firing a transition is to move the tokens from the pre places to the post places.
 ...
 If we use the Token rule but ignore the true concurrency  $\sf TokenRule_{It}$ then the automata it builds  are the interleaving semantics  that would have been constructed had we built the automata directly from a process specification $\mathcal{P}$, hence we have:

\[{\sf TokenRule_{It}(Petri(\mathcal{P}))\sim  Automata(\mathcal{P})}\]

Let us write {\sf A} and {\sf B} for Petri Nets. We can   lift the in-fixed    operations ${\it Op_a}$ on automata to   operations ${\it  Op_n}$ on Petri Nets  so that they obey  the following algebraic rules:

\[{\sf TokenRule_{It}({\sf A}{\it Op_n} {\sf B})\sim  (TokenRule_{It}({\sf A}){\it Op_a} TokenRule_{It}({\sf B})) }\]
...
Our Petri Net semantics  distinguishes concurrent processes from their interleaving and thus captures more information implicit in the process terms. It is easy to model event synchronisation in Petri Nets
...
This can easily be seen in the Petri Net \verb|P| shown below. 
...
\begin{center}\includegraphics[scale=0.38]{P.png}\end{center}
\end{minipage}

\subsection{Owned events}


Although Petri Nets capture more information than automata they do not capture everything from our process language as can be seen by considering  both the Petri Net \verb|P| displayed above and the Petri Net \verb|Q| displayed below. The processes consist of two sequential components and differ only in that events \verb|b| and \verb|t| appear in different sequential components in each process.

\vspace{1ex}\begin{minipage}{0.25\textwidth}
\begin{verbatim}
Q = (a->x->t->STOP|| 
     r->x->b->STOP).
  \end{verbatim}
\end{minipage}\begin{minipage}{0.7\textwidth}
\begin{center}\includegraphics[scale=0.38]{Q.png}\end{center}
\end{minipage}\vspace{1ex}

As currently defined the Petri Net semantics of both processes are identical yet processes \verb|P| and \verb|Q| are not the same.  We describe the difference as being  the events \verb|b| and \verb|t| are \emph{owned} by different sequential processes and this information is lost in the Petri Net semantics as defined.

Clearly we can extend both automata and Petri Net semantics to record the  ownership of events.  This way we have \emph{owned automata} and \emph{owned Petri Nets}.  The two very different representations are now semantically equivalent. We show this my constructing mappings from \emph{owned Petri Nets}  to \emph{owned automata} and back again. The mapping from \emph{owned Petri Nets}  to \emph{owned automata}  can be given simply by extending the {\sf Token Rule} to preserve the ownership of events whereas the mapping in the other direction is given next and is new.
...
\subsection{Two semantically equivalent  views of  the same process specification}
From this point we will assume that our semantics models are annotated by \emph{ownership} and that the Token Rule and the Ownership Rule both preserve the ownership information. Mathematically speaking they are isomorphic.
The advantage of being able to model the same process in two quite distinct semantics allows us to define operations on which ever semantics is most natural and then to project the operation to the other.
...
We can define how to refine a single  event in a process (PetriNet) into a whole process (Petri Net).  Refinement is constructed by applying the \emph{glueing} function used in the definitions of sequential composition and external choice. In essence all that is needed is to glue the pre places of an event to the root of the process that replaces it and then the post places of the event to the end marking of the process.
...
\end{minipage}\begin{minipage}{0.7\textwidth}
\begin{center}
\includegraphics[scale=0.3]{RefSimp.png}\end{center}
\end{minipage}\vspace{1ex}

It is important to realise that event refinement is not defined as syntactic substitution. To do so would require the syntax to be modified and even then  it would either not be able to apply it to processes built via abstraction and simplification or would have to re-compute the process from new syntax. Although we want the same result as if it were constructed via syntactic  substitution,  this is clearly the case with the example above where the net \verb|N| is clearly what we would expect from \verb$(a->STOP)=>(r->STOP||s->STOP)=>(c->STOP)$.

A more complex and   interesting example is given below. It shows that event refinement can construct nets with markings that are not sets but are finite multi sets. Such nets are referred to as {\sf n-safe nets}. Up to now all nets have been {\sf one-safe nets} sometimes referred to as predicate nets.
...
Interestingly the Owners Rule, as defined,  dose not rebuild the n-safe net we started from.  It builds a semantically equivalent net that is 1-safe, see above. It would be interesting to consider if the algorithm could be amended to build n-safe nets.

The same process can be represented by the Petri Nets \verb|X| and \verb|Y| given above.  Which representation is \emph{better} is a matter of personal taste. The Petri Net \verb|X| is smaller which is good but on the other hand not being one-safe prevents us from interpreting  places as predicates and the marking of the place as asserting the truth of the predicate it  represents.


That the Owners Rule converts n-safe nets into "equivalent" one-safe nets came as a surprise and hence prompts the questions: is the rule correct? and what do we mean by correct?
...
\vspace{1ex}\begin{minipage}{0.9\textwidth}
\begin{center}\includegraphics[scale=0.4]{ReachNets.png}\end{center}
\end{minipage}\vspace{1ex}

 In the Petri Net \verb|S| above the events \verb|y| and \verb|c| are unreachable. This is immediately apparent when converted to an automata \verb|S|. Subsequently applying the OwnersRule to the automata builds the Petri Net \verb|R| in which the unreachable events do not appear.
...
Using {\sf owned event} semantics we can \emph{lift} failure semantics nd hence Failure equality from automata to Petri Nets. Given two Petri Nets {\sf P} and {\sf Q} we define them to be Failure equivalent precisely when their automata semantics are Failure equivalent.

\[ \sf P =_{F} Q \triangleq  Token(P)=_{F} Token(Q) \]


This lifting can naturally can be applied to any  process equivalence.
...
\section{Model checking our theory}
Although we reason about processes by  rewriting the operational semantics of the processes and not by rewriting the equational axioms we can make use of the equational axioms to test the theory implicit in the tool.

There are two sections we can use, {\sf operations}  and {\sf equations}.  The operations section consists of a set of terms:
\verb$A~(B=>(a->STOP)).$ consisting of an in-fixed equality, bisimulation, failure or testing and two operands that are terms containing previously defined processes, (closed terms). All the ground equations in the operations section are evaluated and any errors reported

The equations section is similar to the operations but now  the operands can contain process variables, that is open terms :
\verb$X~(Y=>(a->STOP)).$ To evaluate one of these equations the variables need to be replaced by actual processes. These processes are taken from the set of processes defined in the {\sf processes} section.

Evaluating the given equation when the  {\sf processes} section contains 20 processes will result in 400 ($20^2$) ground equations being tested. By this technique it is feasible generate and evaluate thousands of tests. This test generation is far from perfect and results in multiple instances of the same test being generated. Ways to improve this are known but have yet to be implemented.


Adding implication  to the language used in operations and equations will allow congruence relations to be tested an further by adding failure refinement as well as failure equality will allow the Galois Connections mentioned to be tested.
...
Event & \verb$P/{Q/e}$ &  Replaces event {\sf e} by process {\sf Q} \\ 
Refinement && \\ \hline 
Sequential  & ${\sf A=>B}$   &  Behave like  {\sf A} and when {\sf A} terminates behave like {\sf B}\\ 
Composition && \\ \hline
Internal& {\sf A+B}   &  Can behave like {\sf A} or like {\sf B} but which is chosen internally. \\ 

Choice  && Not chosen externally by event synchronisation.\\ \hline
...
Complete Trace   & $\sf A\# B$  & A and B are complete trace equivalent\\ \hline
     
Failure   & $\sf A* B$  & A and B are failure equivalent\\ \hline
 Negation prefix      & $!\_$   &  $\quad\sf A !* B$,  $\quad\sf A !\# B$,  $\quad \sf A !\sim B$\\ \hline 
\end{tabular}


\newpage
\vspace{2ex} \noindent For evaluating equations use the following:

\begin{verbatim}operations  {  
(A||B)=>C ~ (A=>C)||(B=>C)
 }\end{verbatim}

When there are {\sf P} distinct processes defined then equation

\begin{verbatim}equations {  
X=>(Y[]Z) # (X=>Y)[](X=>Z)  
}\end{verbatim}

with its  3 variables will generate $\sf P^3$  equations.
