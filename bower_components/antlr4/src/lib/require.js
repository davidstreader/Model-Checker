...
// INFO Standalone require2()
//      This is a stripped down standalone version of Smoothie's require2
...
//      module has been loaded.
	...
	function require2(identifier, callback) {
		var descriptor = resolve(identifier, true);
		return _require(descriptor);
	}

	function require3(identifier, callback) {
		var descriptor = resolve(identifier, false);
		return _require(descriptor);
	}

	function _require(descriptor, callback) {
...
function resolve(identifier, usePrefix) {
	...
	return {'id':id,'uri':((usePrefix?prefix:"") + uri)};
}

// INFO Exporting require2 to global scope

if (window.require2 !== undefined)
	throw new SmoothieError('\'require2\' already defined in global scope');

try {
	Object.defineProperty(window, 'require2', {'value':require2});
	Object.defineProperty(window.require2, 'resolve', {'value':resolve});
	Object.defineProperty(window.require2, 'paths', {'get':function(){return paths.slice(0);}});

	Object.defineProperty(window, 'require3', {'value':require3});
	Object.defineProperty(window.require3, 'resolve', {'value':resolve});
	Object.defineProperty(window.require3, 'paths', {'get':function(){return paths.slice(0);}});
}
catch (e) {
	...
	window.require2.resolve = resolve;
	window.require2.paths = paths.slice(0);

	window.require3 = require3;
	window.require3.resolve = resolve;
	window.require3.paths = paths.slice(0);
...
function /*load*/(module/*, cache, pwd, source*/) {
	...
	if (typeof module.id !== 'string')
		for (id in module)
			arguments[1]['$'+require2.resolve(id).id] = module[id].toString();
	arguments[2].shift();
}
