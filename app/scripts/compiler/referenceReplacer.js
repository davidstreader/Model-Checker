...
function replaceReferences(processes){
  ...
  function replaceProcess(process, ident, idMap){
    ...
    return replaceNode(process, idMap);
  }
   ...
   */
  function replaceNode(astNode, idMap){
    const rootNode = {node:astNode};
    const fringe = [rootNode,'node'];
    while (fringe.length > 0) {
      ...
      switch (type) {
        ...
        case 'composite':
          fringe.push(astNode);
          fringe.push('process2');
          fringe.push(astNode);
          fringe.push('process1');
          break;
        case 'function':
          fringe.push(astNode);
          fringe.push('process');
          break;
        case 'identifier':
          astNode = replaceIdentifier(astNode, idMap);
          if (astNode.process) {
            astNode = astNode.process;
            fringe.push(parent);
            fringe.push(id);
          }
          break;
   ...
   * @return{ASTNode} - the updated ast node
   */
  function replaceIdentifier(astNode, idMap){
    ...
    idMap[reference] = referenceId++;
    // add a reference to the root of the process ast
    process.reference = idMap[reference];
