...
const Parser = {
  ...
  parseSimpleOperator: function(tokens){
    ...
    while(match !== null){
      ...
      if(this.variableMap[match[0]] === undefined){
        expr = expr.replace(match[0], match[0].substring(1));
        match = expr.match(regex);
        missing = true;
        continue;
      }
      expr = expr.replace(match[0], this.variableMap[match[0]]);
      match = expr.match(regex);
    }
    if (missing) {
      return expr;
  ...
  processUnaryOperator: function(operator, value){
    ...
    const realVal = this.processExpression(value,this.variableMap);
    switch(operator.value){
      ...
      case '!':
        //! is simple. We can just return !expr, as this can be parsed somewhere else.
        return operator.value+value;
      default:
        message = 'Expecting to parse an unary operator but received the operator \'' + operator.value + '\'';
        throw new ParserException(message, operator.location);
    }
  },
