...
function interpretOperation(model){
	...
	for(var i = 0; i < processes.length; i++){
		var name = '#Operation-' + operationsCount + '[' + i + ']';
		defNames.push(name);
		...
		interpretProcess(processes[i], root, defNames[i], defNames[i]);
		processDefinition(name);
	}

	processOperation(operation, defNames[0], defNames[1], model.input, model.isNegated, model.position);
	operationsCount++;
}

function processOperation(operation, defName1, defName2, input, isNegated, position){
	var result = false;
	// process the operation
	if(operation == 'bisimulation'){
		processBisimulation(defName1, defName2);
	}
	else{
		console.error('Tring to interpret invalid operation \'' + operation + '\'.');
	}
	...
	// add result to operations map
	operationsMap[operationCount] = { input: input, result: result, position: position };
}

function processBisimulation(defName1, defName2){
	var graph1 = definitionsMap[defName1].graph;
	var graph2 = definitionsMap[defName2].graph;
